...
async function fetchTeamLastFive(teamId, endDate) {
  const startDate = isoDateDaysAgo(45);
  const data = await fetchJson(`${MLB_BASE}/schedule?sportId=1&teamId=${teamId}&startDate=${startDate}&endDate=${endDate}&hydrate=linescore,team`);
  const games = (data.dates || []).flatMap((d) => d.games || []);
  const finals = games.filter((g) => ['F', 'O', 'R', 'C'].includes(g.status?.codedGameState));
  finals.sort((a, b) => new Date(b.gameDate) - new Date(a.gameDate));
  return finals.slice(0, 5).map((g) => {
    const isAway = g.teams?.away?.team?.id === Number(teamId);
    const us = isAway ? g.teams?.away : g.teams?.home;
    const opp = isAway ? g.teams?.home : g.teams?.away;
    const runsFor = us?.score ?? 0;
    const runsAgainst = opp?.score ?? 0;
    return {
      gamePk: g.gamePk,
      date: g.gameDate,
      opponent: opp?.team?.abbreviation || opp?.team?.name || '',
      homeAway: isAway ? 'A' : 'H',
      result: runsFor > runsAgainst ? 'W' : runsFor < runsAgainst ? 'L' : 'T',
      score: `${runsFor}-${runsAgainst}`,
      runsFor,
      runsAgainst,
      hits: us?.leagueRecord ? null : null
    };
  });
}
...
function compactGame(game) {
  ...
  return {
    ...
    teams: {
      away: { id: game.teams?.away?.team?.id, name: game.teams?.away?.team?.name, abbr: game.teams?.away?.team?.abbreviation, score: game.teams?.away?.score ?? 0, logo: game.teams?.away?.team?.id ? `https://www.mlbstatic.com/team-logos/team-cap-on-dark/${game.teams.away.team.id}.svg` : '' },
      home: { id: game.teams?.home?.team?.id, name: game.teams?.home?.team?.name, abbr: game.teams?.home?.team?.abbreviation, score: game.teams?.home?.score ?? 0, logo: game.teams?.home?.team?.id ? `https://www.mlbstatic.com/team-logos/team-cap-on-dark/${game.teams.home.team.id}.svg` : '' }
    },
    inning: linescore.currentInning || null,
    inningHalf: linescore.inningHalf || null,
    venue: game.venue?.name || null,
    probablePitchers: {
      away: game.teams?.away?.probablePitcher ? { id: game.teams.away.probablePitcher.id, fullName: game.teams.away.probablePitcher.fullName } : null,
      home: game.teams?.home?.probablePitcher ? { id: game.teams.home.probablePitcher.id, fullName: game.teams.home.probablePitcher.fullName } : null
    }
  };
}
  ...
  try {
    ...
    if (pathname === '/api/pregame') {
      ...
      const [awayLast5, homeLast5] = await Promise.all([
        awayId ? fetchTeamLastFive(awayId, etDate).catch(() => []) : Promise.resolve([]),
        homeId ? fetchTeamLastFive(homeId, etDate).catch(() => []) : Promise.resolve([])
      ]);
      return sendJson(res, 200, {
        gamePk: Number(gamePk),
        status: gd.status?.detailedState || 'Preview',
        gameDate: gd.datetime?.dateTime || null,
        venue: gd.venue?.name || null,
        weather: gd.weather || null,
        probablePitchers: gd.probablePitchers || {},
        teams: {
          away: { id: awayId, name: gd.teams?.away?.name, record: gd.teams?.away?.record || null, logo: awayId ? `https://www.mlbstatic.com/team-logos/team-cap-on-dark/${awayId}.svg` : '', last5: awayLast5 },
          home: { id: homeId, name: gd.teams?.home?.name, record: gd.teams?.home?.record || null, logo: homeId ? `https://www.mlbstatic.com/team-logos/team-cap-on-dark/${homeId}.svg` : '', last5: homeLast5 }
        }
      });
    }
