...
function isPregameStatus(status = '') {
  ...
  return t.includes('pre-game') || t.includes('preview') || t.includes('scheduled');
}
...
function logoImg(url, alt) {
  return url ? `<img src="${url}" alt="${alt}" class="team-logo"/>` : '';
}
...
function renderGames() {
  ...
  state.games.forEach((g) => {
    ...
    const countdown = g.status === 'Preview' ? countdownToStart(g.startTime) : '';
    card.innerHTML = `
      <div class="${statusClass(g.status)}">${g.status === 'Final' ? 'FINAL' : g.detailedStatus}</div>
      <div class="team-row">${logoImg(g.teams.away.logo, g.teams.away.abbr)} <span>${g.teams.away.abbr} ${g.teams.away.score}</span> @ ${logoImg(g.teams.home.logo, g.teams.home.abbr)} <span>${g.teams.home.abbr} ${g.teams.home.score}</span></div>
      <div class="muted">${new Date(g.startTime).toLocaleTimeString()} ${countdown}</div>`;
...
async function refreshGame() {
  ...
  const [summary, feed, matchup, buzz, pregame] = await Promise.all([
    ...
    api(`/api/pregame?gamePk=${gamePk}`).catch(() => null)
  ]);
  ...
  const showPregame = isPregameStatus(summary.status) && pregame;
  const pregamePanel = el('pregamePanel');
  if (showPregame) {
    pregamePanel.style.display = 'block';
    const awaySP = pregame.probablePitchers?.away?.fullName || '';
    const homeSP = pregame.probablePitchers?.home?.fullName || '';
    const awayRecord = pregame.teams?.away?.record ? `${pregame.teams.away.record.wins}-${pregame.teams.away.record.losses}` : '';
    const homeRecord = pregame.teams?.home?.record ? `${pregame.teams.home.record.wins}-${pregame.teams.home.record.losses}` : '';
    const awayLast5 = (pregame.teams?.away?.last5 || []).map((g) => `<div class="item">${g.result} ${g.score} ${g.homeAway}${g.opponent}</div>`).join('');
    const homeLast5 = (pregame.teams?.home?.last5 || []).map((g) => `<div class="item">${g.result} ${g.score} ${g.homeAway}${g.opponent}</div>`).join('');
    el('pregameContent').innerHTML = `
      ${pregame.gameDate ? `<div class="item"><b>First pitch:</b> ${new Date(pregame.gameDate).toLocaleString()} ${countdownToStart(pregame.gameDate)}</div>` : ''}
      ${pregame.venue || pregame.weather?.condition || pregame.weather?.temp ? `<div class="item"><b>Venue:</b> ${pregame.venue || ''} ${pregame.weather?.temp ? `· ${pregame.weather.temp}°` : ''} ${pregame.weather?.condition || ''}</div>` : ''}
      ${awaySP || homeSP ? `<div class="item"><b>Probable starters:</b> ${awaySP} ${awaySP && homeSP ? 'vs' : ''} ${homeSP}</div>` : ''}
      ${awayRecord || homeRecord ? `<div class="item"><b>Records:</b> ${awayRecord} ${awayRecord && homeRecord ? '@' : ''} ${homeRecord}</div>` : ''}
      <div class="pregame-grid">
        <div><div class="muted">${logoImg(pregame.teams?.away?.logo, pregame.teams?.away?.name || 'away')} ${pregame.teams?.away?.name || ''} last 5</div>${awayLast5}</div>
        <div><div class="muted">${logoImg(pregame.teams?.home?.logo, pregame.teams?.home?.name || 'home')} ${pregame.teams?.home?.name || ''} last 5</div>${homeLast5}</div>
      </div>`;
  } else {
